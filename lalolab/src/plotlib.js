/*
	...
	Usage:
		plot1 = new Plot() ; 
		plot1.setScalePlot ( minX, maxX, Nsamples, scaleY)	// scaleY is a factor of scaleX
		
		plot1.plot( f [, color_index ] ) 
		...
...
function selectlanguage(lang) {
	var url = window.location.href;
	var newurl = url.replace("/"+ getLanguage() + "/","/"+ lang + "/");
	window.location.replace(newurl);
	
}
// Header ////////////
function makeHeader(draftlevel) {
	if ( typeof(draftlevel) === "undefined") 
		var draftlevel = 1.0;
	...
	var html = "";
	if (lang == "fr" ) 
		html = '<p  class="booktitle"><a href="index.html" class="booktitle">Le Livre Interactif de l\'apprentissage Automatique</a></p>';
	else
		html = '<p  class="booktitle"><a href="index.html" class="booktitle">An Interactive Journey into Machine Learning</a></p>'; 		
		
	html += '<p class="langbuttons"><a onclick="selectlanguage(\'en\');">[EN]</a><a onclick="selectlanguage(\'fr\');">[FR]</a></p>';
	
	html += '<div class="container">';
	if (lang == 'fr') {
		html += '<p class="navigation"><a href="contents.html">Sommaire</a></p>';
		html += '<p class="jumpto">Aller directement <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'words\');">aux mots</a>, <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'pictures\');">aux images</a> ou <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'maths\');">aux maths</a></p>';
	}
	else {
		html += '<p class="navigation"><a href="contents.html">Table of contents</a></p>';
		html += '<p class="jumpto">Jump directly to <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'words\');">the words</a>, <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'pictures\');">the pictures</a> or <a style="cursor: pointer; text-decoration: underline;" onclick="jump(\'maths\');">the maths</a></p>';
	}
	
	if ( draftlevel > 0 ) {
		html += '<label class="draft" style="background-color: rgba(255,0,0,' + draftlevel + ');" title="This page can be incomplete and/or include unchecked and possibly wrong material.">Still under construction</label>';
	}
	
	html += '<hr class="header-hr">';
	html += '</div>';
	hdr.innerHTML = html;		
...
function jump(str) {
	// Jump (scroll) to object taking into account the header
	var target = document.getElementById(str);
	window.scrollTo(0, target.offsetTop - target.offsetHeight - document.getElementById("header").offsetHeight);
}	


function bookifypage (  ) {
	var links = document.getElementsByTagName("a");
	if ( window.location.href.indexOf("#") >= 0 )
		var thispage =  window.location.href.substr(0,window.location.href.indexOf("#"));
	else
		var thispage =  window.location.href;
	for (var i in links ) {
		var a = links[i];
		if ( a.href && a.href != "" && (a.href.indexOf("#") < 0 || a.href.substr(0,a.href.indexOf("#")) != thispage) ) {
//			console.log(a.href,a.href.indexOf("#"));
			var href = a.href;
			a.setAttribute("data-ref", a.href) ;
	...
	// make F.Lauer link in footer
	document.getElementById("footer").innerHTML = '<hr class="footer-rule">Written by <a target="_blank" href="http://www.loria.fr/~lauer/">F. Lauer</a>, 2014.';
}
function turnpage ( event ) {
	preloadpage(event.target.getAttribute("data-ref"));
	/*document.body.style.transformOrigin = "0% 50%";
	document.body.style.transition = "all 1s";
	document.body.style.transform = "rotateY(45deg)";*/
	//page.style.transition = "all 0.5s";
	if ( event.ctrlKey ) {
		// open a new window
		window.open ( event.target.getAttribute("data-ref") );
	}
	else {
		// turn page in same window
		page.style.left = "-1000px";
		setTimeout(function () {
			page.style.left = "0px";// reset this page style if coming back here with the back button
			window.location.assign(event.target.getAttribute("data-ref")) ; 
			}
			, 500);
	}
}
...
/*
	newpage = document.createElement("iframe");
	newpage.onload = function () {
		// replace window by the preloaded one
		setTimeout( function() {
		document.head.innerHTML = newpage.contentDocument.head.innerHTML;
		document.body.innerHTML = newpage.contentDocument.body.innerHTML;
		}, 1000);
	};
	newpage.src = url;
...
/*
	...
	xhr.open ('GET', url, true); // asynchroneous
	//xhr.onload = function () {
		//setTimeout(function () {document.write(xhr.responseText);}, 500);
	//};
	xhr.send();
*/
}
...
Diagram.prototype.arrow = function (x,y,w, h, color, txt, txtcolor, opacity, angle ) {
	...
	this.shapes.push( ["arrow",  x, y, w, h, color, txt, txtcolor, opacity, angle  ] ) ;
	
	this.redraw();
}
Diagram.prototype.arrowTo = function (x,y,x2, y2, h, color, txt, txtcolor, opacity ) {
	if(typeof(opacity) === 'undefined')
		var opacity = 0.6;
	if(typeof(txtcolor) === 'undefined')
		var txtcolor = 0;
	...
	var dx = (x2-x) * canvas.width;
	var dy = (y2-y) * canvas.height; 
	var angle = (180/Math.PI)*Math.atan(dy/dx);
	var w = Math.sqrt( dx*dx + dy*dy ) / canvas.width;
	this.shapes.push( ["arrow",  x, y, w, h, color, txt, txtcolor, opacity, angle  ] ) ;
	
	this.redraw();
}
Diagram.prototype.point = function (x,y,w, color, txt, txtcolor, opacity ) {
	if(typeof(opacity) === 'undefined')
		var opacity = 0.6;
	if(typeof(txtcolor) === 'undefined')
		var txtcolor = 0;
	...
	this.shapes.push( ["point",  x, y, w, w, color, txt, txtcolor, opacity  ] ) ;
	
	this.redraw();
}
Diagram.prototype.label = function (x,y, id ) {	
	
	this.shapes.push( ["label",  x, y, 0, 0, 0, "", 0, 1, id  ] ) ;
...
Diagram.prototype.clear = function () {
	...
	this.selectedShape = -1; // for mousemove
	this.selectedShapes = new Array();	// for user
	this.disabledShapes = new Array(); // indexes of shapes that cannot be selected
...
Diagram.prototype.redraw = function () {
	...
	for ( n = 0; n < this.shapes.length; n++) {
		...
		if( txt != "" ) { 
			...
			if ( words.length == 1 ) {
				while ( ctx.measureText(txt).width < w * canvas.width - 35 ) {
					txtsize += 2;
					ctx.font = txtsize + "pt sans-serif";
				}
...
function gaussian(x) {
	return (1.0 / (sigma * Math.sqrt(2 * Math.PI))) * Math.exp(-(x - mu)*(x-mu) / (2 * sigma * sigma));
}

function plotexample() {
	
	setScalePlot(-5, 15, 300, 3);
	plot(Math.sin, 2);
}
...
function addfct() {
	...
	var sigma = Math.random()*5;
	plot(function gaussian(x) {
			return Math.exp(-(x - mu)*(x-mu) / (2 * sigma * sigma));
		}
		, -1);	// -1 for default color (automatically increased)
}

// 2D
function f2d(x) {
	return Math.sin(x[0] * x[0] + x[1] ) ; 
}
function plotexample2D() {
	
	levelcurve(f2d, 0);
}
function plotexample2Dcolor() {
...
function setcolortransparentgradient(ctx, color_idx, opacity,size) {
	opacity = 1.0;
	var gradient = ctx.createLinearGradient(50,50,400,400);
	...
	case 0: 
		...
		break;
	case 1: 
		gradient.addColorStop(1,"rgba(0,0,200," + opacity +" )");
		ctx.strokeStyle = "rgba(0,0,200," + opacity +" )";
		break;
	case 2: 
		gradient.addColorStop(1,"rgba(200,0,0," + opacity +" )");
		ctx.strokeStyle = "rgba(200,0,0," + opacity +" )";
		break;
	case 3: 
		...
		break;
	case 4: 
		gradient.addColorStop(1,"rgba(200,0,200," + opacity +" )");
		ctx.strokeStyle = "rgba(200,0,200," + opacity +" )";
		break;
	case 5: 
...
function showsolution(n) {
	var sol = document.getElementById('solution' + n);
	var btn = document.getElementById('btnsolution' + n);
	
	if ( btn.innerHTML == "Hide the answer") {
		sol.style.visibility= 'hidden';
		btn.innerHTML = "Show the answer";
	}
	else {
		sol.style.visibility= 'visible';
		 btn.innerHTML = "Hide the answer";
	}
}

function showproof(n) {
	var proof = document.getElementById('proof' + n);
	var btn = document.getElementById('btnproof' + n);
	
	if ( btn.innerHTML == "Hide the proof") {
		proof.style.visibility= 'hidden';
		proof.style.position= 'absolute';		
		btn.innerHTML = "Show the proof";
	}
	else {
		proof.style.position= 'relative';
		proof.style.visibility= 'visible';
		btn.innerHTML = "Hide the proof";
	}
}
